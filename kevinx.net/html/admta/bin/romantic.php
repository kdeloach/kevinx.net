...
function preferences_form4($monitor, $pickup){
...
<option value="Level F">Level F, I-IV-I-V (and/or V7)-I, all inversion, root only or solid chords in the bass, hands together</option>
      ...
      <fieldset><legend>Romantic Selection</legend>
      ...
      <fieldset><legend>Contemporary Selection</legend>
...
// Form output instructions ---------------
  ...
  function form4(&$pdf, $row){
    ...
    // Length of study
    $pdf->setXY(172, 29);
    $pdf->Cell(24.5, LH, $lengthstudy, NO_BORDER, CELL_NEXTLINE, LEFT_ALIGN);
	...
	// 0 - 82.5                         diff=29
	// 1 - 111.5
    $pdf->setXY(88 + $musicreq*30.5, 73);
	$pdf->SetFontSize(18);
    $pdf->Cell(5, LH, 'x', NO_BORDER, CELL_RIGHT, CENTER_ALIGN);*/
	
	// NO Rating checkbox
    $pdf->setXY(73.5, 255.5);
	$pdf->SetFontSize(18);
  ...
  function form4_M(&$pdf, $teacher_info, $count=0){
    ...
    while( $row=$stmt->fetch() ){ $y += $nl; $count++;
      ...
      if( $i == 20 && $count+1<$stmt->rowCount() ){
		  ...
		  return form4_M($pdf, $teacher_info, $count);
      }
